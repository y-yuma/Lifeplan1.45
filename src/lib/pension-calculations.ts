...
export function calculatePensionMonths(basicInfo: BasicInfo): {
  ...
  const workingYears = basicInfo.currentAge - basicInfo.workStartAge;
  ...
  if (ageIn200304 >= basicInfo.workStartAge) {
    monthsUntil200304 = Math.round((ageIn200304 - basicInfo.workStartAge) * 12);
  }
...
export function calculatePension(
  ...
  // 5. 繰上げ/繰下げ調整率を計算
  const adjustmentRate = calculatePensionAdjustmentRate(basicInfo.pensionStartAge);
  ...
  if (basicInfo.willWorkAfterPension) {
    // 年金受給開始時点での就労収入を推定
    const pensionStartYear = basicInfo.startYear + (basicInfo.pensionStartAge - basicInfo.currentAge);
